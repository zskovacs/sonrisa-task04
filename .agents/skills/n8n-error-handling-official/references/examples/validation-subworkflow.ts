...
// Schema-driven validation pattern. The validator runs as an IIFE inside a
// Set Fields expression. See the comment inside the IIFE for the perf
// rationale. The validation is schema-specific: edit the REQUIRED_SCHEMA
// constant and the per-field checks below it for your endpoint's schema.
//
// Output shape (for the Respond to Webhook node and any downstream consumer):
//   { result: { valid: true,  validationError: null } }                                       // happy path
//   { result: { valid: false, validationError: <human-readable summary>,                       // sad path
//               details: { <field>: <message>, ... },
//               requiredSchema: <the schema, echoed back> } }
//
// The Respond to Webhook node maps that into the standard 4xx body
// (`{ error: "validation_error", message, details, request_schema }`) when
// invalid, or `{ success: true }` when valid. Status code is derived from
// `$json.result.valid`.
...
const respond = node({
  ...
  config: {
    ...
    parameters: {
      respondWith: 'json',
      responseBody: expr(`{{ $json.result.valid ? JSON.stringify({ success: true }) : JSON.stringify({ error: "validation_error", message: $json.result.validationError, details: $json.result.details, request_schema: $json.result.requiredSchema }) }}`),
      options: {
        responseCode: expr(`{{ $json.result.valid ? 200 : 400 }}`),
      },
    },
    position: [880, 300],
  },
...
export default workflow('schema-validator', 'Schema Validator')
  .add(webhookTrigger)
  .to(validateSchema)
  .to(respond)
