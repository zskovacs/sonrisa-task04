...
## 2026-09-14 — Admin plan failure and test-isolation corrections

The admin plan initially left malformed database configuration handling implicit despite switching from management services to direct PageModel queries. Review required narrow pre-query validation and HTTP tests for both absent and malformed configuration, with generic 503 responses and no raw error/configuration disclosure. Review also found that cross-owner aggregate assertions could race other tests' temporarily committed profiles and alerts. The new admin integration tests use a nonparallel xUnit collection so baseline-plus-fixture totals are deterministic relative to the suite. Both plan corrections were approved before implementation. See [admin validation evidence](../evidence/reviews/2026-09-14-operational-admin-validation.md).

The induced admin timeout test exposed NpgsqlExecutionStrategy wrapping a TimeoutException in InvalidOperationException. The fix recognizes only an immediate database/timeout inner exception, leaving programming failures visible. Task review also found that merely parsing a connection string does not guarantee its required settings are usable; a new missing-host HTTP test reproduced the provider failure, and pre-query host validation restored the safe 503 contract. All seven admin tests and scoped re-review then passed.

## 2026-09-14 — Integrated validation corrections

The validation plan initially required restoration without spelling out interruption-safe recovery. Independent review required a saved remote baseline, explicit recovery operations, per-scenario try/finally restoration and a fresh meaningful graph/identity/settings comparison before another scenario or commit. The corrected process was used for all controlled runtime runs; the final sanitized export matched the original artifact byte-for-byte.

Live refused-database and blank-Host checks contradicted an assumption that management pages shared the admin area’s safe outage behavior: alerts/settings returned 500. Red HTTP tests reproduced EF’s wrapped database exception and Npgsql’s missing-host failure. Both services now use the same narrow host/failure guards already present in admin, and four regression cases passed. No architecture or schema change was needed. The new successful-edit HTTP test additionally verifies ignored forged ownership fields, persisted typed conditions and stale/invalid post atomicity. Scoped reviews approved both changes; [integrated evidence](../evidence/reviews/2026-09-14-end-to-end-validation.md) records 83/83 guarded tests and the live checks.

The first abbreviated RTK test summary omitted relational skips and did not produce the requested TRX. The controller replaced that evidence with an explicit proxy invocation and inspected both console and TRX counters: 64 passed/19 skipped without external DB test configuration, then 83 passed/zero skipped with verified DEV. No unconfigured run is presented as a complete relational pass.

Final whole-branch review approved with no findings after independently checking TRX counts, export parity/restoration, local links and evidence boundaries. The accepted limitations remain unchanged, and the final documentation milestone was not started.


## 2026-09-15 — Independent reviews reconciled against the current revision

The supplied reviews assessed `e667447`; the controller checked them against `ce8b76c` and rejected stale claims that the retrospective and documentation milestone were missing. Git confirmed 22 removed prompt records and historical number reuse, correcting the first review's count and the index's incomplete explanation. The curated archive remains an explicit limitation rather than being relabeled as a complete transcript. Earlier best-effort/manual-runtime decisions were not silently reversed into scheduling, history resets or durable delivery.

Local reproduction confirmed the percent-encoded Node builder path failure in a directory containing spaces. Shared destination vectors exposed management accepting a local-only email domain and the evaluator accepting malformed local dots that the Email preparer rejected. The correction retains FluentValidation built-in rules with the existing runtime-compatible mailbox contract. Controller review also replaced an unnecessary custom regex wrapper with the built-in Matches rule and bounded regex input through the length rule. An additional trailing-newline hypothesis was tested against Node and rejected: the non-multiline JavaScript pattern already rejected that input, so the Email preparer was preserved.

Plan review required explicit control/length/omission rules for the new Slack alert-name text and complete per-finding traceability; both were added before implementation. [The assessment and checks](../evidence/reviews/2026-09-15-independent-review-corrections.md) record the selected corrections, deferred recommendations and validation boundary, including the locally corrected workflow artifact not being deployed to shared n8n.

The unexpected-error regression uses the real `/alerts` endpoint with a throwing owner-resolution dependency before any database query. A test draft included a prohibited example connection value; controller review removed it in favor of the connection-free DI failure. Error checks now inspect both log and trace payloads and require the safe correlation to equal the actual trace ID. Review also strengthened the hosting-lifetime test with an explicit safe exported marker; no timeout is claimed merely from that review hypothesis.

The first combined Release run caught a regression missed by the focused validator checks: an existing rendered-settings test expected the original mailbox error message. The new rule had unnecessarily changed that copy. Restoring the original message fixed the focused test without changing the stricter mailbox contract or weakening the assertion; final combined results are recorded in the assessment.
