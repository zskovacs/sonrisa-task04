...
`public.users` contains `id` UUID primary key, optional `email_destination` varchar(254), optional `slack_destination` varchar(80), and a nonempty `revision` UUID. At least one destination is required on a saved profile. Supplied destinations are trimmed/nonblank/bounded. Database trim checks use an explicit Unicode whitespace set matching .NET Trim, independent of PostgreSQL locale. Email uses the same conservative contract in management, evaluation and message preparation: one ASCII dot-atom mailbox with a dotted domain, at most 254 characters, no empty local atoms, malformed domain labels, display names or control characters. Management trims surrounding whitespace before storage but rejects control characters in the original input. Slack is an opaque uppercase alphanumeric channel ID. [Shared test vectors](../tests/fixtures/destination-contract.json) cover normalization and runtime acceptance. Previously stored unsupported mailboxes still produce runtime diagnostics and need correction through settings; this pass does not rewrite existing rows. No password, transport credential, webhook URL or token belongs in this table. The first successful settings save creates the configured owner's row; no fake seeded identity is needed.
