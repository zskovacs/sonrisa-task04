...
For this documentation baseline, inspect the existing directory and preserve local files; create the requested documentation and reserved directories; review consistency, scope, links, and secret exposure; obtain final review; and inspect the exact staged files before the milestone 1 commit. No runtime test or screenshot is expected because implementation has not started. The original initialization request is now retained in [prompt history](../prompts/README.md) at the user's request.

For later material work, follow [AGENTS.md](../AGENTS.md): record qualifying product-related prompts before execution, implement only a reviewed bounded task, validate before acceptance, review each task and the whole change, and record meaningful corrections when they occur. Routine technical housekeeping prompts are excluded from history. Keep durable evidence tied to the revision it validates. Scope growth and speculative architecture require justification, not silent implementation.
