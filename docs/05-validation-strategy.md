...
## SMTP-only extension acceptance

The [SMTP specification](superpowers/specs/2026-09-14-smtp-delivery-design.md) adds channel routing to the existing selected-ID workflow. Verify the forward delivery-state constraint, preserved legacy Unsupported rows, shared claim concurrency, exact SMTP recipient validation, acceptance versus unknown result handling, and pinned Slack branch regression. The controlled fixture must enter the existing normalizer/evaluator with no unrelated matches. One actual SMTP4DEV capture plus a repeat from the claim entry must show one message total. Confirm inactive/default-empty workflows, exact sanitized export parity and no credential material. SMTP capture proves server acceptance, not production inbox placement or milestone 6 retry/circuit completion. See [actual SMTP evidence](../evidence/reviews/2026-09-14-smtp-delivery-validation.md).
