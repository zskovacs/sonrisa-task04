...
**Current runtime: an approved simplification to one n8n workflow.** The management application owns alert/user configuration; n8n owns USGS normalization, native bounded deduplication, typed matching across owners, and best-effort Slack delivery. Five total attempts apply to each notification; exhaustion is visible and later notifications continue. The replacement remains inactive and manually operated. [ADR-012](docs/adr/ADR-012-use-n8n-native-runtime-state.md), the [specification](docs/superpowers/specs/2026-09-14-runtime-simplification-design.md) and [runbook](n8n/README.md) define the correction. Email remains required in the next channel milestone through the same workflow. Historical PostgreSQL runtime persistence and the SMTP extension remain documented in their original commits/evidence; their architecture is superseded.
...
The current milestone uses public USGS all-hour data and an explicit synthetic fixture path through the same processing logic. Native deduplication is bounded and does not promise globally atomic claims or permanent physical-earthquake identity. Delivery is best-effort with bounded per-notification retries and continuation. Product queues, delivery lifecycle state and recovery processors are rejected; email is required next in the same flow. See the [runtime contract](docs/07-runtime-contract.md). The [initial runtime validation](evidence/reviews/2026-09-14-first-runtime-validation.md) and [SMTP validation](evidence/reviews/2026-09-14-smtp-delivery-validation.md) remain historical evidence, not proof of the replacement's native dedup/retry behavior.
