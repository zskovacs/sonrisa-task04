...
**Implemented: milestone 5, first end-to-end n8n alert workflow.** Persisted alert configuration now drives manual USGS ingestion, typed matching and one explicitly selected Slack delivery. EF owns the two runtime tables; all three shared DEV workflows remain inactive. One synthetic notification was acknowledged by Slack and recorded as sent, and repeating the same intent stopped without sending again. See the [runtime specification](docs/superpowers/specs/2026-09-14-first-runtime-design.md), [workflow runbook](n8n/README.md), and [actual validation record](evidence/reviews/2026-09-14-first-runtime-validation.md). The existing Razor Pages management UI and OpenTelemetry setup remain unchanged.
...
The current milestone uses the public USGS one-hour feed, a reserved deterministic fixture path, n8n-owned typed matching and one explicitly selected Slack intent. All workflows remain inactive. Deduplication guarantees `(source, external_id)` identity, not physical-earthquake identity after USGS preferred-ID changes. Automatic retries, queue draining, email execution, attempts/circuits and operational UI remain future milestones. See the [runtime contract](docs/07-runtime-contract.md) and [current validation](evidence/reviews/2026-09-14-first-runtime-validation.md).
