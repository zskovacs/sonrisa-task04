...
**Current runtime: one inactive n8n workflow with Slack and Email branches.** The management application owns alert/user configuration; n8n owns USGS normalization, native bounded deduplication, typed matching across owners, channel fan-out, and best-effort transport. Each transport receives up to five total native attempts; exhaustion is visible and later notifications continue. PostgreSQL stores non-secret product configuration, including destinations, while transport credentials remain in n8n. The [Email validation](evidence/reviews/2026-09-14-email-channel-validation.md) confirms SMTP4DEV submission and capture, not delivery to an external inbox. The workflow remains manually operated. [ADR-012](docs/adr/ADR-012-use-n8n-native-runtime-state.md), the [Email specification](docs/superpowers/specs/2026-09-14-email-channel-design.md) and [runbook](n8n/README.md) define the current boundary.
...
Production application-runtime, n8n product-workflow, and migration access must use distinct least-privilege roles. The current application DEV credential is the explicit exception in [ADR-007](docs/adr/ADR-007-accept-current-dev-database-access.md). Only reviewed EF migrations own schema changes. The original configuration DEV migrations are `20260914125733_InitialAlertConfiguration`, `20260914140740_SharedUserNotificationDestinations`, and `20260914143004_EnforceUnicodeTrimmedConfiguration`. Forward migration `20260914184146_RemoveObsoleteRuntimeState` removed the earlier runtime tables. Email required no schema or application change: `users` continues to hold the non-secret shared Email/Slack destinations and `alerts` continues to hold alert configuration. See the [runtime contract](docs/07-runtime-contract.md). The application never migrates at startup. No roles, unrelated schema, or n8n storage are managed here.
...
The suite covers validation, mapping, ownership, revision conflicts, shared profile changes, atomic visibility/rollback, rendered HTTP status forms, antiforgery and telemetry configuration/privacy. Focused model/migration tests cover removal of obsolete runtime tables, while guarded configuration SQL tests cover parameterized all-owner selection. Exact workflow logic and live n8n execution are validated separately. See [actual evidence](evidence/reviews/2026-09-14-alert-configuration-validation.md) for results, browser/container checks and remaining verification limits; a test plan alone is not evidence.
...
The current workflow uses public USGS all-hour data and an explicit synthetic fixture path through the same processing logic. Native deduplication is bounded and does not promise globally atomic claims or permanent physical-earthquake identity. Slack and Email are best-effort, with bounded per-notification native retries, discard after exhaustion, and continuation. Product queues, delivery lifecycle state and recovery processors are rejected. Email added one routing branch with a small alert-name projection/pass-through exception; it did not change selection or matching. See the [runtime contract](docs/07-runtime-contract.md) and [Email validation](evidence/reviews/2026-09-14-email-channel-validation.md).
