...
Milestones 2–5 and the historical SMTP extension are committed at `e1dc337`; the later runtime simplification remains the accepted architecture. The current approved milestone is [Email as a second notification branch](prompts/026-add-email-channel-to-existing-n8n-workflow.md), under the [approved specification](docs/superpowers/specs/2026-09-14-email-channel-design.md) and [reviewed plan](docs/superpowers/plans/2026-09-14-email-channel.md). The one inactive `Sonrisa - Process Alerts - DEV` workflow owns native bounded deduplication, typed evaluation and best-effort dispatch. Slack and Email each retry a notification up to five total attempts, visibly discard only an exhausted item, and continue later items. PostgreSQL remains configuration-only; do not add queues, delivery persistence, recovery workers, schedules, application-side sending, or an operational admin UI. The Email validation uses SMTP4DEV capture, which proves SMTP submission/capture rather than external inbox delivery. No further generic design approval is required.
...
Use meaningful conventional-style commit messages. Messages such as `update`, `changes`, `fix stuff`, and `WIP` are unacceptable. Follow the nine [planned milestone commits](docs/01-plan.md#planned-milestone-commits), deliberately resequenced during milestone 4 brainstorming. Document the reason before changing their sequence; never create empty commits for future milestones. The current task is milestone 6; use the user-requested commit `feat: add email notification channel to n8n workflow`. Preserve earlier milestone and SMTP commits. Durable queues/recovery/circuits remain rejected MVP scope. Use Git history to establish when milestones actually occurred.
