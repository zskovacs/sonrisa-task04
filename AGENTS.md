...
The completed Email milestone is `a198778`. The current authorized milestone is the [operational admin view](prompts/027-add-operational-admin-view.md), under the [specification](docs/superpowers/specs/2026-09-14-operational-admin-view-design.md) and [reviewed plan](docs/superpowers/plans/2026-09-14-operational-admin-view.md). Add only read-only cross-owner product configuration visibility at `/admin`, `/admin/users` and `/admin/alerts`; normal management remains scoped to the configured MVP owner. Authentication remains deferred: admin pages are not a production security boundary. PostgreSQL schema, ASP.NET runtime ownership, existing telemetry and the one inactive `Sonrisa - Process Alerts - DEV` workflow remain unchanged. Runtime executions, failures, retries and integration diagnostics remain in n8n. No queues, delivery persistence, recovery workers, runtime APIs, sending services or n8n client. The user authorizes unattended design, implementation, validation and the milestone commit without routine approval. Leave `feat/operational-admin-view` checked out; do not merge or start the next milestone.
...
[ADR-001](docs/adr/ADR-001-use-n8n-for-orchestration.md) selects n8n orchestration. [ADR-012](docs/adr/ADR-012-use-n8n-native-runtime-state.md) supersedes ADR-005/011's product runtime persistence and durable delivery requirements: PostgreSQL owns configuration, while n8n owns technical deduplication, typed matching, channel dispatch, bounded retries and execution state. The one primary flow has Slack and Email branches with per-notification failure isolation and reviewed exports. Do not add product event/notification queues, recovery workers, circuits or a delivery ledger without a new product requirement.
...
Use meaningful conventional-style commit messages. Messages such as `update`, `changes`, `fix stuff`, and `WIP` are unacceptable. Follow the nine [planned milestone commits](docs/01-plan.md#planned-milestone-commits), deliberately resequenced during milestone 4 brainstorming. Document the reason before changing their sequence; never create empty commits for future milestones. The current task is milestone 7; use the user-requested commit `feat: add operational admin view`. Branch from the completed current HEAD and leave `feat/operational-admin-view` checked out without merging. Preserve earlier milestone and SMTP commits. Durable queues/recovery/circuits remain rejected MVP scope. Use Git history to establish when milestones actually occurred.
