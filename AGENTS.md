...
Milestones 2–5 and the SMTP extension are committed at `e1dc337`. The current user-approved task is the [runtime architecture correction](prompts/025-simplify-n8n-runtime-architecture.md), under the [approved specification](docs/superpowers/specs/2026-09-14-runtime-simplification-design.md) and [reviewed implementation plan](docs/superpowers/plans/2026-09-14-runtime-simplification.md). PostgreSQL returns to product configuration; n8n owns normalization, native bounded deduplication, typed evaluation and best-effort dispatch. Replace the three inactive workflows with one inactive `Sonrisa - Process Alerts - DEV`. Slack retries each notification up to five total attempts, then visibly discards it and continues with subsequent items. Remove the applied runtime tables only through the approved, reviewed forward EF migration; preserve historical migrations and commits. Email remains required in the next runtime milestone as another branch of this same architecture. Do not implement email, schedules, operational UI, authentication, queues, delivery persistence or recovery workers in this correction. No further generic design approval is required.
...
Use meaningful conventional-style commit messages. Messages such as `update`, `changes`, `fix stuff`, and `WIP` are unacceptable. Follow the nine [planned milestone commits](docs/01-plan.md#planned-milestone-commits), deliberately resequenced during milestone 4 brainstorming. Document the reason before changing their sequence; never create empty commits for future milestones. The current task corrects milestone 5's runtime architecture. Use the user-requested forward commit `feat: implement first end-to-end n8n alert workflow`; preserve the earlier milestone and SMTP commits. Email is the required next channel milestone; durable queues/recovery/circuits are rejected MVP scope. Use Git history to establish when milestones actually occurred.
