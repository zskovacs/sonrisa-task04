...
Milestones 2–5 are complete, most recently first runtime at `b9cf171`. The current user-authorized task is the [SMTP email extension](prompts/024-add-smtp-email-delivery.md), following its [approved specification](docs/superpowers/specs/2026-09-14-smtp-delivery-design.md) and [reviewed plan](docs/superpowers/plans/2026-09-14-smtp-delivery.md). Extend the existing selected-ID delivery workflow with a Slack/email branch; do not create a fourth workflow. Preserve ADR-011's n8n evaluation, replay-safe writes and source identity limitations, shared user destinations and EF schema ownership. All three workflows remain inactive. Automatic retries, backlog draining, attempts/circuits, authentication and operational UI are outside this bounded extension. No further generic design approval is required.
...
Use meaningful conventional-style commit messages. Messages such as `update`, `changes`, `fix stuff`, and `WIP` are unacceptable. Follow the nine [planned milestone commits](docs/01-plan.md#planned-milestone-commits), deliberately resequenced during milestone 4 brainstorming. Document the reason before changing their sequence; never create empty commits for future milestones. The current task is the approved SMTP-only follow-up after milestone 5; full automatic delivery recovery, circuits and operational UI remain later work. Use `feat: add SMTP notification delivery` for this focused extension; do not amend the completed milestone commit. Use Git history to establish when milestones actually occurred.
