...
The completed operational admin milestone is `eaa588b`, following Email `a198778` and the simplified n8n runtime `c38f2a3`. The current authorized milestone is [end-to-end validation](prompts/028-end-to-end-validation.md), under its [specification](docs/superpowers/specs/2026-09-14-end-to-end-validation-design.md) and [reviewed plan](docs/superpowers/plans/2026-09-14-end-to-end-validation.md). The user authorizes unattended validation, controlled integration checks, minimal genuine defect corrections, evidence, review and the milestone commit. Preserve owner-scoped management, read-only cross-owner admin, configuration-only PostgreSQL and one inactive n8n runtime. Authentication, durable delivery, queues, recovery workers and scheduling remain excluded. Temporary fixtures and workflow instrumentation require exact cleanup and verified restoration. Leave `test/end-to-end-validation` checked out; do not merge or start the final documentation milestone.
...
Use meaningful conventional-style commit messages. Messages such as `update`, `changes`, `fix stuff`, and `WIP` are unacceptable. Follow the nine [planned milestone commits](docs/01-plan.md#planned-milestone-commits), deliberately resequenced during milestone 4 brainstorming. Document the reason before changing their sequence; never create empty commits for future milestones. The current task is milestone 8; use the user-requested commit `test: validate end-to-end behavior and failure scenarios`. The user explicitly permitted branching from current `main` after verifying the completed milestones; the new branch starts at `eaa588b`. Leave `test/end-to-end-validation` checked out without merging. Preserve earlier milestone and SMTP commits. Durable queues/recovery/circuits remain rejected MVP scope. Use Git history to establish when milestones actually occurred.
