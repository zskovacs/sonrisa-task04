...
The completed end-to-end validation milestone is `e667447`, following admin `eaa588b`, Email `a198778` and the simplified n8n runtime `c38f2a3`. The authorized final milestone is [documentation finalization](prompts/029-finalize-submission.md), under the [reviewed plan](docs/superpowers/plans/2026-09-15-finalize-submission.md). Scope is documentation, reconciliation and final repository review only. Preserve owner-scoped management, read-only cross-owner admin, configuration-only PostgreSQL and one inactive n8n runtime. Do not add product behavior, mutate shared DEV, execute workflows or send notifications for documentation. Authentication, durable delivery, queues, recovery workers and scheduling remain excluded. The user explicitly permits branching from current main at `e667447`. Leave `docs/finalize-submission` checked out, create the requested documentation commit, and do not merge or begin another milestone.
...
Use meaningful conventional-style commit messages. Messages such as `update`, `changes`, `fix stuff`, and `WIP` are unacceptable. Follow the nine [planned milestone commits](docs/01-plan.md#planned-milestone-commits), deliberately resequenced during milestone 4 brainstorming. Document the reason before changing their sequence; never create empty commits for future milestones. The final task is milestone 9; use the user-requested commit `docs: finalize runbook, AI review evidence and final retrospective`. Its wording replaces the earlier planned milestone-9 message without changing milestone order. The user explicitly permitted branching from current `main` after predecessor verification; `docs/finalize-submission` starts at completed validation `e667447`. Leave it checked out without merging. Preserve all earlier milestone and SMTP commits. Durable queues/recovery/circuits remain rejected MVP scope. Use Git history to establish when milestones actually occurred.
