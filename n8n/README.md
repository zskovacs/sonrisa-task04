...
The current architecture is [ADR-012](../docs/adr/ADR-012-use-n8n-native-runtime-state.md): one primary `Sonrisa - Process Alerts - DEV` workflow in the existing shared instance at `https://n8n.nasgard.io`. Keep it inactive. Manual execution is sufficient; do not activate unattended polling during milestone 5.

## What the workflow does

Manual source selection → USGS all-hour fetch or explicit fixture → shared canonical normalizer → split events → native within-batch duplicate filter → native previous-execution filter → parameterized configuration SELECT → typed magnitude evaluation/channel expansion → one notification per iteration → Slack or visible unsupported-channel branch. Slack success and exhausted failure both return to the loop, so later notifications continue.
...
Email is required in the next runtime milestone through this same routing boundary. It is visibly unsupported here. Historical SMTP code/evidence may be reused selectively for transport preparation, without its rejected delivery-state machinery.

## Credentials and permissions

Bind `Load owner alert configuration` to the intended `Postgres account` credential targeting `sonrisa_dev`. Bind `Send Slack notification` to `Sonrisa DEV Slack`. The authorized controlled DEV channel is `C0C1QRQDTSN`; actual runtime destinations come from the user's persisted profile, not a hard-coded transport parameter.

Credential values stay inside n8n. The exporter deliberately removes credential references as well as secrets; rebind both nodes after import. Intended product permissions are SELECT on `public.users` and `public.alerts`, without DDL or schema ownership. Current broad DEV access and transport limitations are documented exceptions, not production settings. Do not edit unrelated credentials or n8n internal persistence.
...
For deterministic validation, select `mode=fixture` and supply a reviewed earthquake-shaped fixture JSON. `Select trusted feed path` sets the namespace to `demo.usgs`; provider data cannot impersonate this operator choice. The same downstream normalizer and matching code run. Synthetic Slack text is visibly marked. Before any future schedule is added, connect it only to the live source path; never to mutable fixture selection.

Validate upstream with transport suppressed/mocked first. Before the single real test, inspect all matched channels and prove exactly one authorized Slack notification is eligible, with the intended test alert, destination and credential. Do not debug by repeatedly sending to Slack. Repeat the **full entry** to test deduplication; replaying a downstream send node bypasses duplicate filtering and can resend.
...
Slack settings: `retryOnFail=true`, `maxTries=5`, `waitBetweenTries=5000`, `onError=continueErrorOutput`. The Loop Over Items batch size is one; output 1 processes an item and output 0 completes. Wire both Slack outputs through their diagnostics back to the loop. An exhausted notification is discarded; subsequent items proceed. These are five total attempts, not five retries after an initial attempt.

Events are already seen before querying/sending. A failed database read or discarded Slack notification may therefore never be retried by later polling. Seen events also do not rematch newly created/edited alerts. Ambiguous transport failures can cause lost or duplicate notifications. No queue/recovery worker or exactly-once guarantee exists.
...
Local tests cover exact Code source and export guards. Real execution evidence must separately establish native duplicate behavior, real configuration reads, real USGS, controlled retry exhaustion followed by another item, and the single authorized Slack result. Temporary transport mocks are test instrumentation only: remove them and restore/inspect the real graph before export. Pinning Slack success alone does not validate retries.
