...
const expected = [
  ...
  ['Record unsupported email diagnostic', 'set'], ['Record unsupported channel diagnostic', 'set'],
...
const edges = [
  ...
  'Route notification channel:1>Record unsupported email diagnostic:0', 'Route notification channel:2>Record unsupported channel diagnostic:0',
  ...
  'Record unsupported email diagnostic:0>Process each notification:0',
  'Record unsupported channel diagnostic:0>Process each notification:0',
].sort();
const codeNames = { 'Select trusted feed path': 'select-ingest-input', 'Normalize earthquake records': 'normalize-earthquakes', 'Evaluate typed magnitude alerts': 'evaluate-alerts', 'Prepare safe Slack text': 'prepare-slack-message' };
...
const assertSet = (workflow, name, code) => {
  ...
  const expected = code === 'processing_complete' ? { code } :
    ...
    code === 'email_transport_not_implemented' ? { channel: 'diagnostic', code, ...direct } :
...
export function sanitizeWorkflow(snapshot, kind) {
  ...
  for (const current of snapshot.nodes) {
    ...
    const isSlackSend = current.name === 'Send Slack notification' && current.type === 'n8n-nodes-base.slack';
    requireValue(!current.pinData && (isSlackSend || !Object.hasOwn(current, 'webhookId')), `node contains pins or webhook: ${current.name}`);
  ...
  requireValue(assignment(snapshot, 'Operator feed input', 'mode') === 'live' && assignment(snapshot, 'Operator feed input', 'fixture_json') === '', 'operator defaults must be live/empty');
  requireValue(keys(node(snapshot, 'Operator feed input').parameters, ['mode', 'includeOtherFields', 'assignments']) && node(snapshot, 'Operator feed input').parameters.includeOtherFields === false, 'unsafe operator input');
  requireValue(same(node(snapshot, 'Use live USGS feed?').parameters?.conditions, { options: { caseSensitive: true, leftValue: '', typeValidation: 'strict' }, conditions: [
    { leftValue: '={{ $json.mode }}', rightValue: 'live', operator: { type: 'string', operation: 'equals' } }
  ], combinator: 'and' }), 'unsafe live fixture routing');
  const fetch = node(snapshot, 'Fetch USGS all-hour feed');
  ...
  requireValue(same(node(snapshot, 'Mark live USGS source').parameters, { mode: 'manual', includeOtherFields: false, assignments: { assignments: [
    { id: 'live-source', name: 'source', value: 'usgs', type: 'string' },
    { id: 'live-feed', name: 'feed', value: '={{ $json }}', type: 'object' }
  ] } }), 'unsafe live feed envelope');
  requireValue(same(node(snapshot, 'Split canonical events').parameters, { fieldToSplitOut: 'events', include: 'noOtherFields' }), 'unsafe event split');
  requireValue(same(node(snapshot, 'Deduplicate current feed').parameters, { operation: 'removeDuplicateInputItems', compare: 'selectedFields', fieldsToCompare: 'source,external_id', options: { removeOtherFields: false } }), 'unsafe within-feed deduplication');
  requireValue(same(node(snapshot, 'Deduplicate previous executions').parameters, { operation: 'removeItemsSeenInPreviousExecutions', logic: 'removeItemsWithAlreadySeenKeyValues', dedupeValue: '={{ $json.source + ":" + $json.external_id }}', options: { scope: 'node', historySize: 10000 } }), 'unsafe previous-execution deduplication');
  const pg = node(snapshot, 'Load owner alert configuration');
  ...
  requireValue(same(node(snapshot, 'Process each notification').parameters, { batchSize: 1, options: { reset: false } }), 'unsafe notification loop');
  const route = node(snapshot, 'Route notification channel').parameters;
  ...
  const send = node(snapshot, 'Send Slack notification');
  requireValue(node(snapshot, 'Prepare safe Slack text').onError === 'continueErrorOutput', 'unsafe Slack preparation error path');
